...
class KMeans():
    ...
    def initialize(self, k):
        ...
        '''
        self.centroids = self.data[np.random.choice(self.num_samps, k, replace = False), :]
        return self.centroids
    ...
    def cluster(self, k=2, tol=1e-5, max_iter=1000, verbose=False):
        ...
        centroid_i = self.initialize(k)
        label = self.assign_labels(centroid_i)
        new_centroids, centroid_diff = self.update_centroids(k, label, centroid_i)
        while (num_iter < max_iter) and (diff for diff in centroid_diff > tol):
            label = self.assign_labels(new_centroids)
    ...
    def cluster_batch(self, k=2, n_iter=1, verbose=False):
        ...
        centroid_best = self.centroids
        label_best = self.data_centroid_labels
        for i in range(n_iter):
            inertia, num_iter = self.cluster(k, verbose = verbose)
            if inertia < min_inertia:
                min_inertia = self.inertia
                centroid_best = self.centroids
                label_best = self.data_centroid_labels
        self.centroids = centroid_best
        self.data_centroid_labels = label_best
    ...
    def assign_labels(self, centroids):
        ...
        '''
        new_labels = np.zeros((self.num_samps,))
        for i in range(self.num_samps):
            for j in range(centroids.shape[0]):
                if np.linalg.norm(self.data[i] - centroids[j]) == np.min(self.dist_pt_to_centroids(self.data[i], centroids)):
                    new_labels[i] = j
        self.data_centroid_labels = new_labels.astype('int')
        return self.data_centroid_labels
    ...
    def update_centroids(self, k, data_centroid_labels, prev_centroids):
        ...
        for i in range(k):
            new_centroids[i] = self.data[data_centroid_labels == i].mean(axis = 0)
        self.centroids = new_centroids
    ...
    def compute_inertia(self):
        ...
        inertia = 0
        for i in range(self.num_samps):
            label = self.data_centroid_labels[i].astype('int')
            centroid = self.centroids[label]
            inertia += (self.dist_pt_to_pt(self.data[i], centroid))**2
    ...
    def plot_clusters(self):
        ...
        '''
        color_map = palettable.colorbrewer.diverging.RdBu_10
        for i in range(len(self.centroids)):
            cluster = self.data[self.data_centroid_labels == i]
            plt.scatter(cluster[:, 0], cluster[:, 1], cmap = color_map.mpl_colormap)
            plt.plot(self.centroids[i, 0], self.centroids[i, 1], 'k*')
    ...
    def elbow_plot(self, max_k, n_iter = 1):
        ...
        for i in range(1, max_k + 1):
            ...
            inertia = self.cluster_batch(k = i, n_iter = n_iter)
            clusters.append(i)
    ...
    def replace_color_with_centroid(self):
        ...
        '''
        for i in range(self.num_samps):
            idx = self.data_centroid_labels[i]
            self.data[i, :] = self.centroids[idx.astype('int')]
